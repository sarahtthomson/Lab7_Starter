...
async function getRecipes() {
  ...
  var currentRecipes = JSON.parse(localStorage.getItem('recipes'));
  ...
  return new Promise(async function(resolve, reject) {
    ...
    for(var i=0; i < RECIPE_URLS.length; i++) {
      var currentRecipe = RECIPE_URLS[i];
      ...
      try {
        ...
        var currentFetch = await fetch(currentRecipe);
        ...
        //            "await" again
        var recipesJson = await currentFetch.json();
        // A8. TODO - Add the new recipe to the recipes array
        fetchedRecipes.push(recipesJson);
